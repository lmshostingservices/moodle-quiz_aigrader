...
$plugin->release   = '3.9.6'; // GROUP FILTER: Added group selector dropdown to the AI Essay Grader report page. Trainers can now pick any course group from a dropdown above the essay cards — only submissions from students in that group are loaded. When no group is selected all essays appear as before. Uses a GET-form page reload with a JOIN on {groups_members} in the SQL. New lang strings: filter_group_label, filter_all_groups, filter_apply, filter_clear_group. New CSS for the filter bar (.aigrader-group-filter-bar). No DB schema changes. version.php → 2026070300391.
